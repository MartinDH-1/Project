  ...
  useEffect(()=>{
    ...
    <section className={`${Styles.beneficios}`}>
        ...
        <motion.div
            ...
            data-isOpen={isOpen3}
            className={`${Styles.parent} shadow-lg`}
            onHoverStart={() => setIsOpen3(true)}
            onHoverEnd={() => setIsOpen3(false)}>

            <motion.div layout  className={`${Styles.child}`}>
                <Image src="https://res.cloudinary.com/dihawqrnt/image/upload/f_auto,q_auto/v1/NuevaPagina/icon-beneficios/facilidad_operacion" alt="Facilidad de operacion" fill />
            </motion.div>
        </motion.div>
        <motion.div
            layout
            data-isOpen={isOpen4}
            className={`${Styles.parent} shadow-lg`}
            onHoverStart={() => setIsOpen4(true)}
            onHoverEnd={() => setIsOpen4(false)}>

            <motion.div layout  className={`${Styles.child}`}>
                <Image src="https://res.cloudinary.com/dihawqrnt/image/upload/f_auto,q_auto/v1/NuevaPagina/icon-beneficios/equipo_demo" alt="Equipo demo prueba" fill />
            </motion.div>
        </motion.div>
        <motion.div
            layout
            data-isOpen={isOpen5}
            className={`${Styles.parent} shadow-lg`}
            onHoverStart={() => setIsOpen5(true)}
            onHoverEnd={() => setIsOpen5(false)}>

            <motion.div layout  className={`${Styles.child}`}>
                <Image src="https://res.cloudinary.com/dihawqrnt/image/upload/f_auto,q_auto/v1/NuevaPagina/icon-beneficios/deducible_impuesto" alt="Deducibles de impuestos" fill />
            </motion.div>
        </motion.div>
        <motion.div
            layout
            data-isOpen={isOpen6}
            className={`${Styles.parent} shadow-lg`}
            onHoverStart={() => setIsOpen6(true)}
            onHoverEnd={() => setIsOpen6(false)}>

            <motion.div layout  className={`${Styles.child}`}>
                <Image src="https://res.cloudinary.com/dihawqrnt/image/upload/f_auto,q_auto/v1/NuevaPagina/icon-beneficios/envio_gratis" alt="Envio gratis a todo México" fill />
            </motion.div>
        </motion.div>
    </section>
  );
